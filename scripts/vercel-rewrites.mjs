 ...
 *
 * Bu on beş adres SEO adresleri (§20) ve dinamik segmentle yazılamıyor:
 * şehir adı yol parçasının İÇİNDE geçiyor (`/ankara-astronomi-etkinlikleri`)
 * ve React Router'ın `:param`'ı yalnızca tam bir parçayı karşılıyor.
 *
...
function cityPaths() {
  ...
  const ids = [...data.matchAll(/\bid:\s*'([a-z0-9-]+)'/g)].map((m) => m[1]);
  if (ids.length === 0) throw new Error('şehir listesi okunamadı');
