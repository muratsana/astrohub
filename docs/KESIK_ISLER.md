...
| 1.3 | **İlçe explorer facet'i** | **BİTTİ** | `listings`, `events`, `clubs` tablolarına `district` kolonu; üç spec'e `ilce` facet'i; ortak `DistrictFilterCell`. Süzgeç seçenekleri SABİT LİSTEDEN DEĞİL VERİDEN geliyor — 974 ilçe sunup çoğu tıklamayı boş listeyle bitirmek yerine yalnızca gerçekten kaydı olan ilçeler, sayılarıyla. Hiç ilçe yoksa süzgeç HİÇ ÇİZİLMİYOR (`personalFacets` kuralı). Formlara `DistrictSelect` eklendi. **Saha DIŞARIDA ve bu bir karar**: gözlem sahaları bilerek yaklaşık konum taşıyor (`approx_latitude`), ilçe eklemek konumu birkaç km'ye daraltıp §14.4'ün korumasını kısmen geri alırdı |
| 1.6 | Etkinlik formunda il alanı serbest metindi | **BİTTİ** | 1.3 sırasında bulundu. `ProvinceSelect` ilan formunda ve profilde yazım tekliğini sağlıyordu ama etkinlik formu atlanmış: "Muğla"/"Mugla"/"MUĞLA" ayrı şehirler gibi kaydediliyor, takvimin şehir süzgeci katalogdan üretildiği için aynı il listede birkaç kez görünüyordu |
...
| 15 | Güvenlik, KVKK, telif, kötüye kullanım | 1529–1606 | CSP, RLS, kota, KVKK onayları ve denetim kaydı var. `consume_rate_limit`in `app` şemasına taşınması canlıda kapandı (`20260803094927` + `20260803095601`). **`20260807140000` ile üç borç kapandı**: `search_path` taşımayan üç fonksiyon (gölgeleme saldırısına açıktı), satır başına `auth.uid()` çağıran dört RLS politikası ve bir çift indeks. Kalan borç 208 permissive policy birleştirmesi — yönetim paneli turu o politikaların çoğuna dokunacağı için ERTELENDİ, iki kez iş olmasın |
