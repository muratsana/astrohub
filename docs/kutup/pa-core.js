 ...
 *
 * Hiçbir DOM API'si kullanmaz; hem React bileşenleri hem de vanilla widget'lar
 * aynı bağıntıları paylaşır.
 *
 * ---------------------------------------------------------------------------
 ...
 *
 * cos(enlem) çarpanı: kutup ufuktan enlem kadar yüksektedir, dolayısıyla
 * zenit ekseni etrafındaki bir dönmenin gökyüzündeki karşılığı cos(enlem)
 * kadar küçülür. Aynı çarpan TPOINT gösterme modelinde de vardır.
 *
 * ---------------------------------------------------------------------------
 * DOĞRULANMIŞ BAĞINTILAR
 * (Python'da tam katı-cisim benzetimiyle ve Barrett'ın yayımlanmış
 *  örnekleriyle karşılaştırılarak sınanmıştır.)
 *
 ...
 *
 * İki köşeli parantez tam olarak diktir: sürüklenmenin en duyarlı olduğu
 * saat açısında alan dönmesi sıfırdır ve tersi.
 */
...
export const OMEGA_ARCSEC_MIN = (2 * Math.PI / 86164.0905) * 60 / D2R * 3600; // ≈ 902.464

/** 1 yay dakikası hata → en fazla bu kadar sürüklenme (yay sn/dk). */
export const DRIFT_PER_ARCMIN = OMEGA_ARCSEC_MIN * ARCMIN; // ≈ 0.26252

/**
 * PHD2 / Barrett sabiti: 12/π = 3.8197.
 * Gökyüzünü saatte 15° (ortalama güneş hızı) kabul eder.
 * Yıldız gününe göre tam değer 3.80929'dur; fark %0.27.
 */
...
/**
 * Yükseklik/azimut hatasını gökyüzü düzlemindeki (a, b) vektörüne çevirir.
 * @param {number} altErr yükseklik hatası, yay dakikası
 * @param {number} azErr  azimut hatası (vidada), yay dakikası
 * @param {number} latDeg enlem, derece
 */
export function errorVector(altErr, azErr, latDeg) {
  const a = altErr * ARCMIN;
  const b = azErr * ARCMIN * Math.cos(latDeg * D2R);
  return { a, b, epsArcmin: Math.hypot(a, b) / ARCMIN };
}

/** Deklinasyon sürüklenmesi [yay sn/dk]. Hedefin deklinasyonundan bağımsızdır. */
export function decDrift(a, b, hourAngle) {
  const H = hourAngle * 15 * D2R;
  return OMEGA_ARCSEC_MIN * (a * Math.sin(H) - b * Math.cos(H));
}

/** Sağ açıklık sürüklenmesi (büyük çember) [yay sn/dk]. */
export function raDrift(a, b, hourAngle, decDeg) {
  const H = hourAngle * 15 * D2R;
  return -OMEGA_ARCSEC_MIN * Math.sin(decDeg * D2R) * (a * Math.cos(H) + b * Math.sin(H));
}

/** Alan dönmesi hızı [yay sn/dk]. Guiding bunu DÜZELTEMEZ. */
export function fieldRotation(a, b, hourAngle, decDeg) {
  const H = hourAngle * 15 * D2R;
 ...
 * hizalama hatası. cos δ bölmesi dahil (bu bölme, ölçüm ekvatordan uzakta
 * yapıldığında sonucu şişirir — makalenin 7. bölümüne bakın).
 */
export function errorFromDrift(driftArcsecMin, decDeg) {
  const cd = Math.max(0.02, Math.cos(decDeg * D2R));
  return K_PHD2 * Math.abs(driftArcsecMin) / cd;
}

/** Aynı hesap, cos δ olmadan — geometrik olarak doğru olan. */
export function errorFromDriftTrue(driftArcsecMin) {
 ...
 *
 * @returns {{ra:number[], dec:number[], corr:number[]}} yay saniyesi
 */
export function makeTrace(o) {
  const n = o.n || 130;
  const expSec = o.expSec || 2;
  const dt = expSec / 60;
  const rnd = mulberry32(o.seed || 4242);
  const ra = [], dec = [], corr = [];
  let decTrue = 0, raTrue = 0, dsm = 0, lastDir = 0, blDebt = 0;
  ...
  for (let i = 0; i < n; i++) {
    ...
    const osc = (o.decOsc || 0) * Math.sin(2 * Math.PI * t / (o.decOscP || 100));
    ...
    if (o.guided !== false) {
      ...
      if (Math.abs(dsm) > minMove) {
        ...
        if (o.stiction && dir !== lastDir && lastDir !== 0) applied *= o.stiction;
    ...
    ra.push(raMeas); dec.push(decMeas); corr.push(c);
  }
  return { ra, dec, corr };
}

/** En küçük kareler eğimi (PHD2'nin eğilim çizgisi). */
export function linFit(arr) {
  const n = arr.length;
