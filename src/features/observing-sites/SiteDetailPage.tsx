      ...
      <div className="mt-6 flex flex-col gap-8 lg:flex-row">
        <div className="min-w-0 flex-1">
          ...
          <p className="tabular mt-1 text-sm text-muted-foreground">
            {site.region} · {site.altitude} m rakım · ★ {site.rating.toFixed(1)} (
            {site.reviewCount} değerlendirme)
          </p>

          <div className="mt-3 flex flex-wrap gap-1.5">
            <Badge tone="primary">Bortle {site.bortle}</Badge>
            {site.sqm && <Badge tone="blue">SQM {site.sqm} mag/arcsec²</Badge>}
          ...
          <section className="mt-8">
            ...
            <p className="mb-3 text-sm text-muted-foreground">
              Yol durumu: <span className="text-foreground">{site.roadAccess}</span>
            </p>
            <ul className="grid grid-cols-2 gap-2 sm:grid-cols-3">
              {facilityRows.map(([label, ok]) => (
                ...
                >
                  <span
                    aria-hidden
                    className={ok ? 'text-success' : 'text-muted-foreground/50'}
                  >
                    {ok ? '✓' : '✗'}
                  </span>
        ...
        <aside className="w-full shrink-0 lg:w-72">
          <div className="rounded-2xl border border-border bg-surface-1 p-5 text-sm text-muted-foreground">
            ...
            <p className="mt-2 text-xs leading-relaxed">
              Koordinatlar gizlilik politikası gereği yaklaşık paylaşılır (§15.3).
              Harita görünümü ve yol tarifi, harita katmanı devreye alınınca
              eklenecek.
            </p>
