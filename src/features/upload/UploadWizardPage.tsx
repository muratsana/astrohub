  ...
  return (
    <Container className="py-10 sm:py-14">
      ...
      <div className="rounded-2xl border border-border bg-surface-1 p-5 sm:p-8">
            ...
            <div className="space-y-3">
              {state.exposures.map((row, i) => (
                <div key={i} className="grid grid-cols-[1fr_1fr_1fr_auto] gap-3">
                  ...
                  <Field label={i === 0 ? 'Kare sayısı' : ''} htmlFor={`n-${i}`}>
                    <Input
                      id={`n-${i}`}
                      type="number"
                      min={0}
                  ...
                  <Field label={i === 0 ? 'Pozlama (sn)' : ''} htmlFor={`s-${i}`}>
                    <Input
                      id={`s-${i}`}
                      type="number"
                      min={0}
                  ...
                  <div className="flex items-end pb-0.5">
                    <Button
                      ...
                      onClick={() =>
                        patch({
                          exposures: state.exposures.filter((_, j) => j !== i),
                        })
                      }
                    >
                      ✕
        ...
        {step === 5 && (
          <div className="space-y-5">
            <StepTitle title="İşleme, lisans ve yayın" hint="Önizleme ve onay" />
            ...
            <div className="rounded-xl border border-border bg-surface-2 p-4">
              <h3 className="mb-2 text-sm font-semibold text-foreground">Özet</h3>
        ...
        <div className="mt-8 flex items-center justify-between border-t border-border pt-5">
          ...
          {step < steps.length - 1 ? (
            ...
            <Button disabled title="Depolama altyapısı bağlandığında aktifleşecek">
              Yayımla (yakında)
            </Button>
          )}
        </div>
